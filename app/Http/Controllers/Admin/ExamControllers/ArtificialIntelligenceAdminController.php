...
use Carbon\Carbon;
use App\Models\ArtificialIntelligenceModule;
use App\Models\ExamManage;
use App\Models\ArtificialIntelligenceQuestion;

class ArtificialIntelligenceAdminController extends Controller
{
    public function addArtificialIntelligence(Request $request)
    {
        if ($request->isMethod('POST') && $request->ajax() && Auth::check()) {
            ...
            if (is_exist('course_master', ['id' => $award_id, 'is_deleted' => 'No']) > 0) {
                try {
                    ...
                    $exists = is_exist('exam_artificial_intelligence', $where);
                    if (isset($exists) && $exists > 0) {
                        ...
                        DB::beginTransaction();
                        $assignUpdate = processData(['exam_artificial_intelligence', 'id'], $select);
                        ...
                        if (isset($assignUpdate) && $assignUpdate['status'] === true) {
                            ...
                            if (isset($assginManageUpdate) && $assginManageUpdate['status'] === true) {
                                DB::commit();
                                return json_encode(['code' => 200, 'title' => "Successfully Added", 'message' => 'Artificial intelligence added successfully', "icon" => generateIconPath("success")]);
                            }
                        }
                        DB::rollBack();
                        return json_encode([
                            'code' => 404, 'title' => "Unable to Add Artificial intelligence", 'message' => 'Please Try Again', "icon" => generateIconPath("error")
                        ]);
                    }
                } catch (\Throwable $th) {
                    return $th;
                    DB::rollBack();
                    return json_encode([
                        'code' => 404, 'title' => "Unable to Add Artificial intelligence", 'message' => 'Something Went Wrong', "icon" => generateIconPath("error")
                    ]);
                }
            } else {
                return json_encode([
    ...
    public function getArtificialIntelligenceData($action = '', $id = '')
    {
        if (Auth::check()) {
            $contentData = [];
            ...
            if (isset($action) && !empty($action) && $action === 'edit') {
                $contentData = $this->artificialIntelligenceModule->getArtificialIntelligenceDetails($where);
                return view('admin/exam/edit-artificial-intelligence', compact('contentData'));
            } else {
                $contentData = $this->artificialIntelligenceModule->getArtificialIntelligenceDetails($where);
            }
    ...
    public function updateArtificialIntelligence(Request $request)
    {

        if ($request->isMethod('POST') && $request->ajax() && Auth::check()) {
            $admin_id = Auth::user()->id;
            $artificial_intelligence_id = isset($request->artificial_intelligence_id) ? base64_decode($request->input('artificial_intelligence_id')) : 0;
            $title = isset($request->title) ? htmlspecialchars_decode($request->input('title')) : '';
            ...
            $where = ['id' => $artificial_intelligence_id, 'is_deleted' => 'No'];
            ...
            if (isset($exists) && $exists > 0) {
                ...
                if ($request->hasFile('instruction_file')) {
                    $instruction_file_url =  UploadFiles($instruction_file, 'course/ArtificialIntelligenceDocs', $old_instruction_file);
                ...
                try {
                    ...
                    if (isset($updateSection) && $updateSection['status'] === true) {
                        DB::commit();
                        return json_encode(['code' => 200, 'title' => 'Successfully Updated', "message" => "Artificial intelligence successfully updated", "icon" => generateIconPath("success")]);
                    }
                    DB::rollback();
    ...
    public function deletePdfFile(Request $req)
    {
        if ($req->isMethod('POST') && $req->ajax() && Auth::check()) {
            ...
            $exists = is_exist($table, $where);
            if (isset($exists) && $exists > 0) {
                $getFileData = getData('exam_artificial_intelligence', ['instrcution_file_url'], ['id' => $id, 'is_deleted' => 'No']);
    ...
    public function addArtificialIntelligenceQuestion(Request $request)
    {
        ...
        if ($request->isMethod('POST') && $request->ajax() && Auth::check()) {
            ...
            $exists = is_exist('exam_artificial_intelligence', ['id' => $artificial_intelligence_id, 'is_deleted' => 'No']);
            $Questionexists = is_exist('exam_artificial_intelligence_questions', ['id' => $question_id, 'is_deleted' => 'No']);
            try {
                $request->validate([
            ...
            if (isset($exists) && is_numeric($exists) && $exists > 0) {
                $select = [
                    'artificial_intelligence_id' =>  $artificial_intelligence_id,
                    'question' => $question,
                    'marks' => $marks,
                    'is_deleted' => 'No',
                ];
                ...
                $updateUser = processData(['exam_artificial_intelligence_questions', 'id'], $select, $where);
            ...
            return json_encode(['code' => 404, 'title' => "Artificial intelligence Not Available", 'message' => 'Please Try Again', "icon" => generateIconPath("error")]);
        }
    }
    
    public function editViewArtificialIntelligenceQeustion(Request $request)
    {
        if ($request->isMethod('POST') && Auth::check()) {
            ...
            if (isset($questionID) && !empty($questionID)) {
                $exists = is_exist('exam_artificial_intelligence_questions', ['id' => $questionID, 'is_deleted' => 'No']);
                if (isset($exists) && is_numeric($exists) && $exists > 0) {
                    $QuestionAssign = getData('exam_artificial_intelligence_questions', ['id', 'question', 'marks', 'artificial_intelligence_id'], ['id' => $questionID]);
    ...
    public function deleteArtificialIntelligenceQuestion(Request $req)
    {
        if ($req->isMethod('POST') && $req->ajax() && Auth::check()) {
            $admin_id = Auth::user()->id;
            $id =  isset($req->id) ? base64_decode($req->input('id')) : 0;
            ...
            $table = "exam_artificial_intelligence_questions";
            $where = ['id' => $id];
            $exists = is_exist($table, $where);
            if (isset($exists) && $exists > 0) {
                try {
                    ...
                    $result = deleteRecord(ArtificialIntelligenceQuestion::class, $where, true);
    ...
    public function deleteArtificialIntelligence(Request $req)
    {
        if ($req->isMethod('POST') && $req->ajax() && Auth::check()) {
            $admin_id = Auth::user()->id;
            $id =  isset($req->id) ? base64_decode($req->input('id')) : 0;
            ...
            if (isset($exists) && $exists > 0) {
                try {
                    ...
                    $result = deleteRecord(ArtificialIntelligenceModule::class, $where, true);

                    if (isset($result) && $result['status'] === true) {
                        $questions = getData('exam_artificial_intelligence_questions', ['id'], ['artificial_intelligence_id' => $result['id']]);
                        if(isset($questions) && !empty($questions[0])){
                            foreach($questions as $question){
                                $where = ['id' => $question->id];
                                deleteRecord(ArtificialIntelligenceQuestion::class, $where, true);
                            }
                        }

                        $where = ['course_id' => $course_id, 'exam_type' => '9', 'exam_id' => $result['id']];
