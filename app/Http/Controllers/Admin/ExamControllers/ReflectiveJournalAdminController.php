...
use Carbon\Carbon;
use App\Models\ReflectiveJournalModule;
use App\Models\ExamManage;
use App\Models\ReflectiveJournalQuestion;

class ReflectiveJournalAdminController extends Controller
{

    public function addReflectiveJournal(Request $request)
    {
        if ($request->isMethod('POST') && $request->ajax() && Auth::check()) {
            ...
            $reflective_journals_title = isset($request->reflective_journals_title) ? htmlspecialchars_decode($request->input('reflective_journals_title')) : '';

            try {
                $request->validate([
                    'award_id' => ['required', 'string'],
                    'reflective_journals_title' => [
                        'required', 'min:3',
                        'max:255', 'string'
                    ],
                ],[
                    'reflective_journals_title.min'=>'The title must be at least 3 characters.',
                    'reflective_journals_title.max'=>'The title must be less than 255 characters.',
                ]);
            ...
            if (is_exist('course_master', ['id' => $award_id, 'is_deleted' => 'No']) > 0) {
                try {
                    ...
                    $exists = is_exist('exam_reflective_journals', $where);
                    if (isset($exists) && $exists > 0) {
                        return json_encode([
                            'code' => 404, 'title' => "Already added", 'message' => 'Course reflective journals already added.', "icon" => generateIconPath("warning")
                        ]);
                    }else{
                        $select = [
                            'title' => $reflective_journals_title,
                            'award_id' => $award_id,
                            'created_by' => $admin_id,
                            'is_deleted' => 'No',
                        ...
                        DB::beginTransaction();
                        $assignUpdate = processData(['exam_reflective_journals', 'id'], $select);
                        ...
                        if (isset($assignUpdate) && $assignUpdate['status'] === true) {
                            ...
                            if (isset($assginManageUpdate) && $assginManageUpdate['status'] === true) {
                                DB::commit();
                                return json_encode(['code' => 200, 'title' => "Successfully Added", 'message' => 'Reflective journal added successfully', "icon" => generateIconPath("success")]);
                            }
                        }
                        DB::rollBack();
                        return json_encode([
                            'code' => 404, 'title' => "Unable to Add Reflective Journal", 'message' => 'Please Try Again', "icon" => generateIconPath("error")
                        ]);
                    }
                } catch (\Throwable $th) {
                    return $th;
                    DB::rollBack();
                    return json_encode([
                        'code' => 404, 'title' => "Unable to Add Reflective Journal", 'message' => 'Something Went Wrong', "icon" => generateIconPath("error")
                    ]);
                }
            } else {
                return json_encode([
    ...
    public function getReflectiveJournalData($action = '', $id = '')
    {
        if (Auth::check()) {
            $contentData = [];
            ...
            if (isset($action) && !empty($action) && $action === 'edit') {
                $contentData = $this->reflectiveJournalModule->getReflectiveJournalDetails($where);
                return view('admin/exam/edit-reflective-journals', compact('contentData'));
            } else {
                $contentData = $this->reflectiveJournalModule->getReflectiveJournalDetails($where);
            }
    ...
    public function updateReflectiveJournal(Request $request)
    {

        if ($request->isMethod('POST') && $request->ajax() && Auth::check()) {
            $admin_id = Auth::user()->id;
            $reflective_journals_id = isset($request->reflective_journals_id) ? base64_decode($request->input('reflective_journals_id')) : 0;
            ...
            $where = ['id' => $reflective_journals_id, 'is_deleted' => 'No'];
            ...
            if (isset($exists) && $exists > 0) {
                ...
                if ($request->hasFile('instruction_file')) {
                    $instruction_file_url =  UploadFiles($instruction_file, 'course/ReflectiveJournalDocs', $old_instruction_file);
                ...
                try {
                    ...
                    if (isset($updateSection) && $updateSection['status'] === true) {
                        DB::commit();
                        return json_encode(['code' => 200, 'title' => 'Successfully Updated', "message" => "Reflective journal successfully updated", "icon" => generateIconPath("success")]);
                    }
                    DB::rollback();
    ...
    public function deletePdfFile(Request $req)
    {
        if ($req->isMethod('POST') && $req->ajax() && Auth::check()) {
            ...
            $exists = is_exist($table, $where);
            if (isset($exists) && $exists > 0) {
                $getFileData = getData('exam_reflective_journals', ['instrcution_file_url'], ['id' => $id, 'is_deleted' => 'No']);
    ...
    public function addReflectiveJournalQuestion(Request $request)
    {
        ...
        if ($request->isMethod('POST') && $request->ajax() && Auth::check()) {
            ...
            $reflective_journal_id = isset($request->reflective_journal_id) ? base64_decode($request->input('reflective_journal_id')) : 0;
            ...
            $exists = is_exist('exam_reflective_journals', ['id' => $reflective_journal_id, 'is_deleted' => 'No']);
            $Questionexists = is_exist('exam_reflective_journal_questions', ['id' => $question_id, 'is_deleted' => 'No']);
            try {
                $request->validate([
                    'questionData' => ['required', 'min:3', 'max:2000'],
            ...
            if (isset($exists) && is_numeric($exists) && $exists > 0) {
                $select = [
                    'reflective_journal_id' =>  $reflective_journal_id,
                    'question' => $question,
                    'marks' => $marks,
                ...
                $updateUser = processData(['exam_reflective_journal_questions', 'id'], $select, $where);
            ...
            return json_encode(['code' => 404, 'title' => "Reflective Journal Not Available", 'message' => 'Please Try Again', "icon" => generateIconPath("error")]);
        }
    }
    
    public function editViewReflectiveJournalQeustion(Request $request)
    {
        if ($request->isMethod('POST') && Auth::check()) {
            ...
            if (isset($questionID) && !empty($questionID)) {
                $exists = is_exist('exam_reflective_journal_questions', ['id' => $questionID, 'is_deleted' => 'No']);
                if (isset($exists) && is_numeric($exists) && $exists > 0) {
                    $QuestionAssign = getData('exam_reflective_journal_questions', ['id', 'question', 'marks', 'reflective_journal_id', 'answer_limit'], ['id' => $questionID]);
    ...
    public function deleteReflectiveJournalQuestion(Request $req)
    {
        if ($req->isMethod('POST') && $req->ajax() && Auth::check()) {
            $admin_id = Auth::user()->id;
            $id =  isset($req->id) ? base64_decode($req->input('id')) : 0;
            ...
            $table = "exam_reflective_journal_questions";
            $where = ['id' => $id];
            $exists = is_exist($table, $where);
            if (isset($exists) && $exists > 0) {
                try {
                    ...
                    $result = deleteRecord(ReflectiveJournalQuestion::class, $where, true);
    ...
    public function deleteReflectiveJournal(Request $req)
    {
        if ($req->isMethod('POST') && $req->ajax() && Auth::check()) {
            $admin_id = Auth::user()->id;
            $id =  isset($req->id) ? base64_decode($req->input('id')) : 0;
            ...
            if (isset($exists) && $exists > 0) {
                try {
                    ...
                    $result = deleteRecord(ReflectiveJournalModule::class, $where, true);

                    if (isset($result) && $result['status'] === true) {
                        $questions = getData('exam_reflective_journal_questions', ['id'], ['reflective_journal_id' => $result['id']]);
                        if(isset($questions) && !empty($questions[0])){
                            foreach($questions as $question){
                                $where = ['id' => $question->id];
                                deleteRecord(ReflectiveJournalQuestion::class, $where, true);
                            }
                        }

                        $where = ['course_id' => $course_id, 'exam_type' => '6', 'exam_id' => $result['id']];
