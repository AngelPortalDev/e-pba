...
<main>
    <section class="p-lg-5 py-7">
        <div class="container">
            ...
            @php
                    $exam = $examData[0];
                    // $examData = $examDataFirst[0];
                    $assginExam = $exam['assginment_exam'][0];
                    $draftExam = $exam['draft_exam'];
                    $turnitinExam = $exam['turnitin_exam'];
                    // print_r($turnitinExam);
                    // echo $exam['exam_id'];
                    // echo $exam['course_id'];
                    // echo $exam['student_course_master_id'];
                    $TurnitinExam = DB::table('exam_assignment_turnitin')->where('exam_id',$exam['exam_id'])->where('course_id',$exam['course_id'])->where('student_course_master_id',$exam['student_course_master_id'])->get();
                    if ($draftExam) {
                        if (
                    ...
                    foreach ($assginQuestion as $key => $value) {
                        ...
                        if($attempt_exam == base64_encode('1')){
                            ...
                            $totalObtain += isset($value['assig_anwers'][1]['marks_given']) && !empty($value['assig_anwers'][1]['marks_given']) ?  $value['assig_anwers'][1]['marks_given'] : 0;
                        }
                    }
            ...
            <div class="row">
                <div class="col-xl-12 col-lg-12 col-md-12 col-12 mb-3 mb-xl-0">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    ...
                                    $routeName = in_array(auth()->user()->role, ['admin', 'superadmin']) 
                                        ...
                                        $attempt = $exam['attempt'] ?? 'first'; // 'first' or 'second'
                                    @endphp
                    ...
                    <!-- Card -->
                    @if(!empty($TurnitinExam))
                    <div class="card mb-4">
                        <!-- Card body -->
                        @if($attempt_exam == base64_encode('1') && isset($TurnitinExam[0]) && !empty($TurnitinExam[0]))
                        <div class="card-body py-3">
                            <a href="{{ Storage::disk('local')->url($TurnitinExam[0]->turnitin_file_url) }}"
                                    download="{{ isset($TurnitinExam[0]->turnitin_file_name) ? $TurnitinExam[0]->turnitin_file_name : ' ' }}"><button
                                        class="btn btn-primary ms-2">Similarity Report PDF <i class="bi bi-download me-2"></i></button></a>   
                            
                            &nbsp;&nbsp;&nbsp;
                            <label><b>Similarity Percentage <b></label>: <?=  isset($TurnitinExam[0]->turnitin_marks) ? $TurnitinExam[0]->turnitin_marks : ' ' ?>
                        </div>
                        @elseif($attempt_exam == base64_encode('2') && isset($TurnitinExam[1]) && !empty($TurnitinExam[1]))
                        <div class="card-body py-3">
                            <a href="{{ Storage::disk('local')->url($TurnitinExam[1]->turnitin_file_url) }}"
                                    download="{{ isset($TurnitinExam[1]->turnitin_file_name) ? $TurnitinExam[1]->turnitin_file_name : ' ' }}"><button
                                        class="btn btn-primary ms-2">Similarity Report PDF <i class="bi bi-download me-2"></i></button></a>   
                            
                            &nbsp;&nbsp;&nbsp;
                            <label><b>Similarity Percentage <b></label>: <?=  isset($TurnitinExam[1]->turnitin_marks) ? $TurnitinExam[1]->turnitin_marks : ' ' ?>
                        </div>
                        @endif
                            ...
                            <form id="examMarksForm">
                                ...
                                @foreach ($assginQuestion as $item)
                                    <div class="col-md-12 mb-3">
                                        ...
                                        <div class="row">
                                            <div class="color-blue d-flex align-items-end">
                                                <div class="">
                                                    ...
                                                    @endif
                                                    <input type="number" name="marks[]" class="form-control marks-input" placeholder="Give Marks" max="{{ isset($item['assignment_mark']) ? $item['assignment_mark'] : 0}}" value="{{ isset($item['assig_anwers'][0]['marks_given']) ? $item['assig_anwers'][0]['marks_given'] : 0}}" required disabled style="width:8rem" >
                                                </div>
                                            </div>
                                        ...
                                        <div class="answer-content">
                                            <p class="answer-text" style="white-space: pre-line;">
                                                @if(isset($item['assig_anwers'][1]) && !empty($item['assig_anwers'][1]['answer']))
                                                    {{ $item['assig_anwers'][1]['answer'] }}
                                                @else
                                                    <span class="not-attempted">--Not Attempted--</span>
                                                @endif
                                            </p>
                                            ...
                                            <div class="color-blue d-flex align-items-end">
                                                <div class="">
                                                    @if($draftExam != null)
                                                        <textarea name="suggestion" id="suggestion" cols="45" rows="5" class="form-control mb-2" placeholder="Please give your suggestions...">{{isset($draftExam['suggestion']) != '' ? $draftExam['suggestion'] : ''}}</textarea>
                                                    @endif
                                                    <input type="number" name="marks[]" class="form-control marks-input" placeholder="Give Marks" max="{{ isset($item['assignment_mark']) ? $item['assignment_mark'] : 0}}" value="{{ isset($item['assig_anwers'][1]['marks_given']) ? $item['assig_anwers'][1]['marks_given'] : 0}}" required disabled style="width:8rem" >
                                                </div>
                                            </div>
