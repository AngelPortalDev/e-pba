...
@endphp
@php
$CourseWatchData = getData('student_course_master',['watch_content_id','course_progress','course_expired_on','id','total_course_price','payment_installment_type'],['user_id'=>$StudentId,'course_id'=>$courseDetails[0]['course'][0]['id']],'','id','DESC');

if($CourseWatchData[0]->payment_installment_type == "InstallmentPayment"){
    $CourseWatchNextData = getData('payment_installment',['next_install_date','paid_install_no'],['user_id'=>$StudentId,'course_id'=>$courseDetails[0]['course'][0]['id'],'student_course_master_id'=> $CourseWatchData[0]->id,'paid_install_status'=>'0'],'1','id','DESC');

    $CourseWatchNextDataAmount = getData('payment_installment',['next_install_date','paid_install_amount'],['user_id'=>$StudentId,'course_id'=>$courseDetails[0]['course'][0]['id'],'student_course_master_id'=> $CourseWatchData[0]->id,'paid_install_status'=>'0'],'','','id','DESC');
...
@php
$quizsteper = 1;
$CourseIconData = getData('student_course_master',['watch_content_id','id'],['user_id'=>$StudentId,'course_id'=>$courseDetails[0]['course'][0]['id']],'','id','DESC');
@endphp
...
<a class="d-flex align-items-center h4 mb-0 "
    ...
    $sectionProgress = calculateSectionProgress(
        ...
        $StudentId, 
        $courseDetails[0]['course'][0]['id'], 
        $CourseIconData[0]->id, 
        $CourseIconData[0]->watch_content_id
    );
    @endphp
...
<div class="d-flex align-items-center justify-content-between w-100 mt-2">
    <div class="progress flex-grow-1 me-2" style="height: 6px;">
        <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated"
            ...
            aria-valuenow="{{$sectionProgress['percentage']}}" aria-valuemin="0" aria-valuemax="100">
        </div>
    </div>
    <small class="text-muted fw-semibold" style="white-space: nowrap;">{{$sectionProgress['percentage']}}%</small>
</div>
<!-- Row -->
<!-- Collapse -->
        ...
        @foreach ($sections['sections'][0]['section_manage'] as $key => $section_manage)
            ...
            <a class="mb-0 d-flex justify-content-between align-items-center text-inherit studentAwardCourseTitle tab-link {{ $j === 0 ?  'active' : '' }}"
                ...
                <div class="d-flex align-items-center">
                    <span class="icon-shape bg-green text-primary icon-sm rounded-circle me-2 icon_bg">
                    @php $CourseProgressData = getData('video_progress',['full_check','last_record_time','duration','watchedRanges'],['video_id'=>$video['id'],'course_id' => $courseDetails[0]['course'][0]['id'],'user_id'=>$StudentId,'student_course_master_id'=>$CourseIconData[0]->id]);@endphp
...
<script>
    ...
    function videoDisplay(videoId, vid) {
        ...
        let watchedRanges = [];
        // if (course_id || vid) {
        //     fetch(`/ementor/video-progress/${course_id}/${vid}/${user_id}`)
        //         .then(res => res.json())
        //         .then(data => {
