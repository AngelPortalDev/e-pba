...
<style>
    .sidenav.navbar .navbar-nav .e-men-11 > .nav-link {
...
<script>
    ...
    function assignedCourseList(action) {
        ...
        if (userRole === 'instructor') {
            url = baseUrl + "/ementor/get-purchased-students-list/"+action;
                        ...
                        render: function(row) {
                            ...
                            let badges = '';
                                if (row.allPaidCourses && row.allPaidCourses.length > 0) {
                                    ...
                                    let docInfo = row.docClass && row.docClass[course.scmId] ? row.docClass[course.scmId] : null;

                                    if (docInfo) {
                                        badges = docInfo
                                        ? `<span class="badge" style="background-color:${docInfo.color}; color:#fff;">${docInfo.label}</span>`
                                        : `<span class="badge bg-primary">Not Uploaded</span>`;
                                        docStatus += `<div class="course-badge" style="height:22.5px;">${badges}</div><hr style="color:#fff">`;
                                    }
                                });
                            }
