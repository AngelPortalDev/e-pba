...
@section('content')
    ...
    @php
        ...
        if (
            isset($ementorStudentDataFirstAttempt['studentData'][0]['exam_student']) &&
            is_array($ementorStudentDataFirstAttempt['studentData'][0]['exam_student']) &&
            count($ementorStudentDataFirstAttempt['studentData'][0]['exam_student']) > 0
        ) {

            $stDataFirst = $ementorStudentDataFirstAttempt['studentData'][0]['exam_student'][0];
            $courseDataFirst = $ementorStudentDataFirstAttempt['studentData'][0]['exam_course'][0];
            $awardCoursesFirst = $ementorStudentDataFirstAttempt['studentData'];
            $courseRelatedDataFirst = $ementorStudentDataFirstAttempt['courseData'];
            $courseTitleFirst = $ementorStudentDataFirstAttempt['courseTitle'];
            $courseCategoryFirst = $ementorStudentDataFirstAttempt['courseCategory'];
        } else {
            $url = url()->current();
            $parameters = request()->route()->parameters();
            $userId = $parameters['id'];
            ...
            $stData = json_decode(json_encode($stData), true);
            $courseRelatedDataFirst = isset($ementorStudentDataFirstAttempt['courseData']) ? $ementorStudentDataFirstAttempt['courseData'] : null;
            $courseDataFirst = null;
        }

        if (
            isset($ementorStudentDataHomework['studentData'][0]['exam_student']) &&
            is_array($ementorStudentDataHomework['studentData'][0]['exam_student']) &&
            count($ementorStudentDataHomework['studentData'][0]['exam_student']) > 0
        ) {

            $stDataHomework = $ementorStudentDataHomework['studentData'][0]['exam_student'][0];
            $courseDataHomework = $ementorStudentDataHomework['studentData'][0]['exam_course'][0]['homework_status'];
            $awardCoursesHomework = $ementorStudentDataHomework['studentData'];
            $courseRelatedDataHomework = $ementorStudentDataHomework['courseData'];
            $courseTitleHomework = $ementorStudentDataHomework['courseTitle'];
            $courseCategoryHomework = $ementorStudentDataHomework['courseCategory'];
        } else {
            $url = url()->current();
            $parameters = request()->route()->parameters();
            $userId = $parameters['id'];
            ...
            $stData = json_decode(json_encode($stData), true);
            $courseRelatedDataHomework = isset($ementorStudentDataHomework['courseData']) ? $ementorStudentDataHomework['courseData'] : null;
            $courseDataHomework = null;
        }
        $totalPercentage = 0;
        $activeAttempt = request()->get('attempt'); // default to 'first'
    @endphp
    <div
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    ...
                                    @elseif(Auth()->user()->role == "turnitin-instructor")
                                        <a href="{{route("e-mentor-students-exam")}}" class="btn btn-outline-primary float-end">Back</a>
                                    @else
                    ...
                    <div class="card rounded-3">
                        ...
                        <div class="card-header border-bottom-0 p-0">
                            <div>
                                ...
                                <ul class="nav nav-lb-tab" id="tab" role="tablist">
                                    ...
                                    @endif
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link {{ ($activeAttempt == 'first' || $activeAttempt == 'homework') ? '' : 'active' }}" id="exam-tab" data-bs-toggle="pill" href="#exam"
                                            role="tab" aria-controls="exam" aria-selected="false"
                                            tabindex="-1">Exam</a>
                                    </li>
                                    @if ($courseDataHomework != null)
                                            @php
                                            ...
                                            $course = DB::table('student_course_master')
                                                ...
                                                ->where('course_id', base64_decode($course_id))
                                                ->where('user_id', base64_decode($user_id))
                                            ...
                                            @if ($course && $course->category_id != 1)
                                                ...
                                                if(!empty($awardCourseIds)){    
                                                    $studentCourseMaster = getData('student_course_master', ['preference_id', 'preference_status'], ['id' => $ementorStudentDataHomework['studentData'][0]['student_course_master_id']]);
                                                ...
                                                $commaSeparated = implode(',', $awardCourseIds->toArray());
                                                $records = DB::table('exam_remark_master')
                                                    ->select('course_id', 'exam_type')
                                                    ->where([
                                                        // 'student_course_master_id' => $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id'],
                                                        'user_id' => $ementorStudentDataHomework['studentData'][0]['user_id'],
                                                ...
                                                // Group by course_id
                                                $grouped = $records->groupBy('course_id');

                                                $courseAttemptStatus = [];

                                                foreach ($grouped as $courseId => $examRecords) {
                                                    // Count how many times each exam_type appears (only 1 and 2)
                                                    $typeCounts = $examRecords->groupBy('exam_type')->map(function ($group) {
                                                        return $group->count();
                                                    });
                                                    // echo $typeCounts;
                                              
                                                    $totalAttempts = $typeCounts->sum();

                                                    if ($totalAttempts > 0) {
                                                        $courseAttemptStatus[$courseId] = 'homework_attempt_display';
                                                    } else {
                                                        $courseAttemptStatus[$courseId] = 'no_attempt';
                                                    }
                                                }

                                                // Optional flags
                                                $hasHomeworkAttempt = in_array('homework_attempt_display', $courseAttemptStatus);
                                                @endphp
                                            @else
                                                @php
                                                    $hasHomeworkAttempt = '';
                                                    ...
                                                    $completedExamCount = DB::table('exam_remark_master')
                                                        ->where([
                                                            // 'student_course_master_id' => $ementorStudentDataHomework['studentData'][0]['student_course_master_id'],
                                                            'user_id' => $ementorStudentDataHomework['studentData'][0]['user_id'],
                                                            'is_cheking_completed' => '2',
                                                    ...
                                                    $hasHomeworkAttempt = !empty($completedExamCount);      
                                                @endphp
                                            @endif
                                    @else
                                        @php   
                                        ...
                                        @endphp
                                    @endif
                                    @if($hasHomeworkAttempt)
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link {{ $activeAttempt == 'homework' ? 'active' : '' }} " id="homework-tab" data-bs-toggle="pill" href="#homework"
                                            role="tab" aria-controls="homework" aria-selected="false"
                                            tabindex="-1">Homework</a>
                                    </li>
                                    @endif
                                    @if ($courseDataFirst != null)
                                            @php $course = DB::table('student_course_master')
                                                ->join('course_master', 'student_course_master.course_id', '=', 'course_master.id')
                                                ->where('student_course_master.id', $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id'])
                                                ->select('course_master.category_id', 'student_course_master.course_id')
                                                ->first();
                                            @endphp
                                            @if ($course && $course->category_id != 1)
                                                ...
                                                if(!empty($awardCourseIds)){    
                                                    ...
                                                    $studentCourseMaster = getData('student_course_master', ['preference_id', 'preference_status'], ['id' => $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id']]);
                                                    if($studentCourseMaster[0]->preference_status == '0'){
                                                        if($studentCourseMaster[0]->preference_id != null){
                                                            $preferenceIds = $studentCourseMaster[0]->preference_id;
                                                            $preferenceIdsArray = explode(',', $preferenceIds);
                                                ...
                                                $records = DB::table('exam_remark_master')
                                                    ...
                                                    ->where([
                                                        'student_course_master_id' => $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id'],
                                                        'user_id' => $ementorStudentDataFirstAttempt['studentData'][0]['user_id'],
                                                        'is_cheking_completed' => '2',
                                                    ])
                                                    ->whereIn('course_id', $awardCourseIds)
                                                    ->where('exam_type', '!=', 3) 
                                                ...
                                                foreach ($grouped as $courseId => $examRecords) {
                                                    ...
                                                    $type1Count = $typeCounts[1] ?? 0;
                                                    $type2Count = $typeCounts[2] ?? 0;

                                                    if ($type1Count > 1 || $type2Count > 1) {
                                                        $courseAttemptStatus[$courseId] = 'second_attempt_display';
                                                    } elseif ($type1Count === 1 || $type2Count === 1) {
                                                        $courseAttemptStatus[$courseId] = 'first_attempt_display';
                                                    } else {
                                                        $courseAttemptStatus[$courseId] = 'no_attempt';
                                                    }
                                                }
                                                ...
                                                // Optional flags
                                                $hasSecondAttempt = in_array('second_attempt_display', $courseAttemptStatus);
                                                $hasFirstAttempt = in_array('first_attempt_display', $courseAttemptStatus);
                                    ...
                                    @else
                                        ...
                                        @endphp
                                    @endif
                                    @if($hasFirstAttempt || $hasSecondAttempt)
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link {{ $activeAttempt == "first" ? 'active' : '' }}" id="attempt-tab" data-bs-toggle="pill" href="#attempt"
                                            role="tab" aria-controls="attempt" aria-selected="false"
                                            tabindex="-1">Checked Answer Sheet</a>
                                    </li>
                                    @endif
                        ...
                        <div class="card-body">
                            <div class="tab-content" id="tabContent">
                                        ...
                                        @if (isset($courseRelatedData['purchasedCourse']) && count($courseRelatedData['purchasedCourse']) > 0)
                                            @foreach ($courseRelatedData['purchasedCourse'] as $purchaseCourseData)
                                                <div class="d-flex align-items-center mb-3">
                                                    @if($purchaseCourseData->category_id == 1)
                                                        @php $panelUrl = 'ementor/student-award-ementor-panel/'.base64_encode($purchaseCourseData->course_id).'/'. base64_encode($purchaseCourseData->userId);@endphp
                                                    @else
                                                        @php $panelUrl = 'ementor/student-master-ementor-panel/'.base64_encode($purchaseCourseData->course_id).'/'. base64_encode($purchaseCourseData->userId);@endphp
                                                    @endif
                                                    <div>
                                                        <a href="{{ url($panelUrl)}}">
                                                    ...
                                                    <div class="ms-3">
                                                        ...
                                                        <div class="mt-2">
                                                            ...
                                                            <br>
                                                            @if($purchaseCourseData->adjusted_expiry != '')
                                                            <h5 class="badge mt-2" style="background: #a30a1b; color: #ffffff">
                                                                {{ isset($purchaseCourseData->adjusted_expiry) ? "Access Until " . ' ' . \Carbon\Carbon::parse($purchaseCourseData->adjusted_expiry)->format('d-m-y') : '' }}
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                <!-- Exams  -->
                                <div class="tab-pane fade {{ ($activeAttempt == 'first' || $activeAttempt == 'homework') ? '' : 'show active' }}" id="exam" role="tabpanel"
                                ...
                                <div class="tab-pane fade {{ $activeAttempt == 'homework' ? 'show active' : '' }}" id="homework" role="tabpanel"
                                    ...
                                    @if ($courseDataHomework != null)
                                        @php
                                            ...
                                            $studentCourseMaster = DB::table('student_course_master')
                                                ->where([
                                                    'id' => $ementorStudentDataHomework['studentData'][0]['student_course_master_id'],
                                                    'user_id' => $ementorStudentDataHomework['studentData'][0]['user_id'],
                                                    'course_id' => base64_decode($course_id),
                                                ])
                                                ->latest()
                                        ...
                                        @else --}}
                                            <div class="card border mb-4">
                                                ...
                                                <div class="card-body p-4">
                                                    ...
                                                    @php
                                                        $examDatasHomework = [
                                                            'stData' => $ementorStudentDataHomework['studentData'][0]['exam_student'][0],
                                                            'courseDatas' => $ementorStudentDataHomework['studentData'][0]['exam_course'][0],
                                                            'awardCoursesData' => $ementorStudentDataHomework['studentData'],
                                                            'courseRelatedData' => $ementorStudentDataHomework['courseData'],
                                                            'courseTitle' => $ementorStudentDataHomework['courseTitle'],
                                                            'courseId' => $ementorStudentDataHomework['courseId'],
                                                            'courseCategorys' => $ementorStudentDataHomework['courseCategory'],
                                                            'ementorStudentDatas' => $ementorStudentDataHomework
                                                            
                                                        ];
                                                        // print_r($examDatasHomework);
                                                    @endphp
                                                    
                                                    @php $ementorStudentDatas = $examDatasHomework['ementorStudentDatas'];  
                                                        $courseDatas = $examDatasHomework['courseDatas'];
                                                        $awardCoursesDatas = $examDatasHomework['awardCoursesData'];
                                                        $courseCategorys = $examDatasHomework['courseCategorys'];
                                                        // echo "<pre>";
                                                        // print_r($ementorStudentDataHomework);
                                                    @endphp
                                                    ...
                                                    <div class="accordion accordion-flush" id="accordionExample">                                                     
                                                            <div class="accordion accordion-flush" id="accordionExample">
                                                                    ...
                                                                    @if($courseCategorys != '1')
                                                                        @foreach($awardCoursesDatas as $key => $exam)
                                                                        @if($exam['exam_type'] === 10)
                                                                        <div class="border p-3 rounded-3 mb-2" id="headingOne{{$key}}">
                                                                        <h3 class="mb-0 fs-4">
                                                                            <a href="#"
                                                                                class="d-flex align-items-center text-inherit active"
                                                                                data-bs-toggle="collapse"
                                                                                data-bs-target="#collapseOne{{$key}}" aria-expanded="{{ $key == 0 ? 'true' : 'false' }}"
                                                                                ...
                                                                                <ul class="ps-3">
                                                                                    ...
                                                                                    <li class="mb-2">
                                                                                            ...
                                                                                            @if (Auth::user()->role === 'instructor' || Auth::user()->role === 'sub-instructor')
                                                                                                <a href="{{ url('/ementor/answersheet', ['exam_id' => base64_encode($exam['id']), 'exam_type' => base64_encode(10), 'user_id' => base64_encode($exam['user_id']), 'student_course_master_id' => base64_encode($exam['student_course_master_id'])]) }}"
                                                                                                    class="fw-semibold homeworkSection ">{{ isset($exam_homework['homework_title']) ? html_entity_decode($exam_homework['homework_title']) : '' }}
                                                                                                    <i
                                                                                                        class="bi bi-arrow-right fw-bold"></i></a>
                                                                                            @else
                                                                                                <a href="{{ url('/admin/answersheet', ['exam_id' => base64_encode($exam['id']), 'exam_type' => base64_encode(10), 'user_id' => base64_encode($exam['user_id']), 'student_course_master_id' => base64_encode($exam['student_course_master_id'])]) }}"
                                                                                                    class="fw-semibold homeworkSection ">{{ isset($exam_homework['homework_title']) ? html_entity_decode($exam_homework['homework_title']) : '' }}
                                                                                                    {{-- ({{ isset($exam_homework['percentage']) ? $exam_homework['percentage'] : 0 }}%) --}}
                                                                                                    <i
                                                                                                        class="bi bi-arrow-right fw-bold"></i></a>
                                                                                            @endif
                                                                                    </li>
                                                                    ...
                                                                    @else
                                                                        ...
                                                                        <h3 class="mb-0 fs-4">
                                                                            <a href="#"
                                                                                ...
                                                                                <span
                                                                                    class="me-auto">{{ isset($courseDatas['course_title']) ? $courseDatas['course_title'] : '' }}
                                                                                    {{-- <span class="badge bg-success-soft ms-2">Pass</span> --}}
                                                                                </span>
                                                                                <span class="collapse-toggle ms-4">
                                                                                    <i class="fe fe-chevron-down"></i>
                                                                        ...
                                                                        <div id="collapseOne1" class="collapse show"
                                                                            ...
                                                                            <div class="pt-2">
                                                                                <ul class="ps-3">
                                                                                    @foreach ($ementorStudentDatas['studentData'] as $key =>  $exam)
                                                                                    @php  $examtype = $exam['exam_type']; 
                                                                                    @endphp 
                                                                                    
                                                                                    @if($exam['exam_course'][0]['homework_status'][0] && $course_id )
                                                                                    @php
                                                                                        $examtype = $exam['exam_type'];
                                                                                        $exam_homework = isset(
                                                                                        $exam['exam_course'][0][
                                                                                    ...
                                                                                    <li class="mb-2">
                                                                                            ...
                                                                                            @if (Auth::user()->role === 'instructor' || Auth::user()->role === 'sub-instructor')
                                                                                                <a href="{{ url('/ementor/answersheet', ['exam_id' => base64_encode($exam['exam_course'][0][
                                                                                                'homework_status'][0]['id']), 'exam_type' => base64_encode(10), 'user_id' => base64_encode($exam['user_id']), 'student_course_master_id' => base64_encode($exam['student_course_master_id'])]) }}"
                                                                                                    class="fw-semibold homeworkSection">{{ isset($exam_homework['homework_title']) ? html_entity_decode($exam_homework['homework_title']) : '' }}
                                                                                                    <i
                                                                                                        class="bi bi-arrow-right fw-bold"></i></a>
                                                                                            @else
                                                                                                <a href="{{ url('/admin/answersheet', ['exam_id' => base64_encode($exam['exam_course'][0][
                                                                                                'homework_status'][0]['id']), 'exam_type' => base64_encode(10), 'user_id' => base64_encode($exam['user_id']), 'student_course_master_id' => base64_encode($exam['student_course_master_id'])]) }}"
                                                                                                    class="fw-semibold homeworkSection">{{ isset($exam_homework['homework_title']) ? html_entity_decode($exam_homework['homework_title']) : '' }}
                                                                                                    {{-- ({{ isset($exam_homework['percentage']) ? $exam_homework['percentage'] : 0 }}%) --}}
                                                                                                    <i
                                ...
                                <div class="tab-pane fade {{ $activeAttempt == 'first' ? 'show active' : '' }}" id="attempt" role="tabpanel"
                                    aria-labelledby="attempt-tab">
                                        @if ($courseDataFirst != null)
                                            @php 
                                            $hasFirstAttempt = '';
                                            $hasSecondAttempt = '';
                                            ...
                                            @endphp
                                            @if ($course && $course->category_id != 1)
                                                @php $awardCourseIds = DB::table('master_course_management')
                                                    ->where('award_id', $course->course_id)
                                                    ->where(['optional_course_id' => null, 'is_deleted' => 'No'])
                                                    ->pluck('course_id');   
                                    ...
                                    <ul class="nav nav-lb-tab" id="tab" role="tablist">
                                        @if ($hasSecondAttempt)
                                            {{-- Show BOTH First Attempt and Second Attempt --}}
                                            <li class="nav-item" role="presentation">
                                                <a class="nav-link active" id="first-attempt-tab" data-bs-toggle="pill" href="#first-attempt"
                                                    role="tab" aria-controls="first-attempt" aria-selected="false" tabindex="-1">
                                                    First Attempt
                                                </a>
                                            </li>
                                            <li class="nav-item" role="presentation">
                                                <a class="nav-link" id="second-attempt-tab" data-bs-toggle="pill" href="#second-attempt"
                                                    role="tab" aria-controls="second-attempt" aria-selected="false" tabindex="-1">
                                                    Second Attempt
                                                </a>
                                            </li>
                                        @elseif ($hasFirstAttempt)
                                            {{-- Show only First Attempt --}}
                                            <li class="nav-item" role="presentation">
                                                <a class="nav-link active" id="first-attempt-tab" data-bs-toggle="pill" href="#first-attempt"
                                                    role="tab" aria-controls="first-attempt" aria-selected="false" tabindex="-1">
                                                    First Attempt
                                    ...
                                    {{-- <div class="card-body"> --}}
                                    <div class="tab-content" id="tabContent">
                                        <div class="tab-pane fade show active" id="first-attempt" role="tabpanel" aria-labelledby="first-attempt-tab">
                                            {{-- Student profile  --}}
                                            @if ($courseDataFirst != null)
                                            @php
                                                ...
                                                $studentCourseMaster = DB::table('student_course_master')
                                                    ->where([
                                                        'id' => $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id'],
                                                        'user_id' => $ementorStudentDataFirstAttempt['studentData'][0]['user_id'],
                                                        'course_id' => base64_decode($course_id),
                                                    ])
                                                    ->latest()
                                            ...
                                            @endphp
                                                ...
                                                <div class="card border mb-4">
                                                    ...
                                                    <div class="card-body p-4">
                                                        ...
                                                        @php
                                                            $examData = [
                                                                'stData' => $ementorStudentDataFirstAttempt['studentData'][0]['exam_student'][0],
                                                                'courseData' => $ementorStudentDataFirstAttempt['studentData'][0]['exam_course'][0],
                                                                'awardCourses' => $ementorStudentDataFirstAttempt['studentData'],
                                                                'courseRelatedData' => $ementorStudentDataFirstAttempt['courseData'],
                                                                'courseTitle' => $ementorStudentDataFirstAttempt['courseTitle'],
                                                                'courseId' => $ementorStudentDataFirstAttempt['courseId'],
                                                                'courseCategory' => $ementorStudentDataFirstAttempt['courseCategory'],
                                                                'ementorStudentData' => $ementorStudentDataFirstAttempt,
                                                                'attempt_exam' => base64_encode('1'),
                                                                'studentCourseMasterId'=> $ementorStudentDataFirstAttempt['studentCourseMasterId']

                                                            ];
                                                        @endphp
                                                        @if($courseCategoryFirst != '1')
                                                            @include('frontend.teacher.master-course-attempt', $examData)
                                                        @else
                                                            @include('frontend.teacher.award-course-attempt', $examData)
                                                        @endif
                                        ...
                                        <div class="tab-pane fade " id="second-attempt" role="tabpanel" aria-labelledby="second-attempt-tab">
                                            @if ($courseDataFirst != null)
                                            @php
                                                $studentCourseMasterSec = DB::table('student_course_master')
                                                    ->where([
                                                        'id' => $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id'],
                                                        'user_id' => $ementorStudentDataFirstAttempt['studentData'][0]['user_id'],
                                                        'course_id' => base64_decode($course_id),
                                                    ...
                                                    ->latest()
                                                    ->first();
                                                $examRemarkMasterSec = DB::table('exam_remark_master')
                                                    ->where([
                                                        'student_course_master_id' => $ementorStudentDataFirstAttempt['studentData'][0]['student_course_master_id'],
                                                        'user_id' => $ementorStudentDataFirstAttempt['studentData'][0]['user_id'],
                                                        'course_id' => base64_decode($course_id),
                                            ...
                                            @endphp
                                                ...
                                                <div class="card border mb-4">
                                                    ...
                                                    <div class="card-body p-4">
                                                        ...
                                                        @if(isset($studentCourseMasterSec) && !empty($studentCourseMasterSec) && $studentCourseMasterSec->exam_attempt_remain == '0')
                                                                @if ($studentCourseMasterSec->exam_remark == 0)
                                                                    <h4>Final Result : <span
                                                                            class="text-danger">Failed</span>
                                                                    </h4>
                                                                @elseif($studentCourseMasterSec->exam_remark == 1)
                                                                    <h4>Final Result : <span
                                                                            class="text-success">{{ $studentCourseMasterSec->exam_perc }}%
                                                                            (Pass)</span></h4>
                                                                @endif
                                                        @endif
