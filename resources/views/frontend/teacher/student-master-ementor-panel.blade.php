...
{{-- @php
    $doc_verified = getData('student_doc_verification',['english_score','identity_is_approved','edu_is_approved','identity_doc_file','edu_doc_file','resume_file','edu_trail_attempt','identity_trail_attempt','english_test_attempt'],['student_id'=>$StudentId]);
@endphp
...
@php
    ...
    $CourseId   = base64_decode($urlSegments[count($urlSegments) - 2]); // second last segment
    $StudentId  = base64_decode($urlSegments[count($urlSegments) - 1]); // last segment    
    $totalLocks = '';
    // $DocumentVerified = getDocumentVerificationStatus($CourseId);
    $doc_verified = getData('student_doc_verification',['english_score','identity_is_approved','edu_is_approved','identity_doc_file','edu_doc_file','resume_file','edu_trail_attempt','identity_trail_attempt','english_test_attempt','edu_athe_approved','edu_level','student_id','edu_master_approved'],['student_id'=>$StudentId]);

    // $CourseMaster = getData('course_master',['category_id','course_title'],['id'=>$CourseId]);
    $CourseMaster = getData('course_master',['category_id','course_title','course_old_price','course_final_price','id','installment_amount','no_of_installment'],['id'=>$CourseId]);

    $player = "video";

    $canAccess = getDocumentStatusClass($CourseMaster[0], $doc_verified[0],$player,null);
    $CourseWatchDatas = getData('student_course_master',['watch_content_id','course_progress','course_expired_on','id','total_course_price','payment_installment_type'],['user_id'=>$StudentId,'course_id'=>$CourseId],'','id','DESC');

    // print_r($CourseWatchData[0]->id);
    $installmentCount = $CourseMaster[0]->no_of_installment;

    if($CourseWatchDatas[0]->payment_installment_type == "InstallmentPayment"){
        $CourseWatchNextData = getData('payment_installment',['next_install_date','paid_install_no'],['user_id'=>$StudentId,'course_id'=>$CourseId,'student_course_master_id'=> $CourseWatchDatas[0]->id,'paid_install_status'=>'0'],'1','id','DESC');

        $CourseWatchNextDataAmount = getData('payment_installment',['next_install_date','paid_install_amount'],['user_id'=>$StudentId,'course_id'=>$CourseId,'student_course_master_id'=> $CourseWatchDatas[0]->id,'paid_install_status'=>'0'],'','','id','DESC');
        ...
        if($totalPaid == $CourseWatchDatas[0]->total_course_price){
            $expiryDate = '';
...
<?php
    ...
    $VideoData = '';
    $ContentData = '';
    foreach($MasterCourseData as $key => $masterdata){
        if($key == 0){
            ...
            if (isset($sectionData[0]->section_id)) {
                    ...
                    if ($VideoData->count() ==  0){
                        $ContentData = getData('course_content_docs', [ 'file', 'docs_title', 'id', 'doc_file_name'], ['section_id' => $sectionData[0]->section_id, 'is_deleted' => 'No']);
                    }
                }
        }
    }
?>
@if (
    $sectionData instanceof \Illuminate\Support\Collection && $sectionData->isNotEmpty() &&
    (
        ($VideoData instanceof \Illuminate\Support\Collection && $VideoData->isNotEmpty()) ||
        ($ContentData instanceof \Illuminate\Support\Collection && $ContentData->isNotEmpty())
    )
)
                                    ...
                                    <div class="simplebar-content student-award-panel-scrollbar" style="padding: 0px;">
                                         @php
                                                ...
                                                if (
                                                    ...
                                                    $courseName = getOrTranslate('Course', $course_id, 'course_title', $course[0]->course_title ?? '', $locales);
                                                }

                                                $CourseWatchData = getData('student_course_master',['watch_content_id','course_progress','id'],['user_id'=>$StudentId,'course_id'=>$course_id],'','id','DESC');

                                                $CourseIconData = getData('student_course_master',['watch_content_id','id'],['user_id'=>$StudentId,'course_id'=>$course_id],'','id','DESC');

                                                $watchContentIds = isset($CourseIconData[0]) && !empty($CourseIconData) ? $CourseIconData[0]->watch_content_id : '';
                                                $courseIconId = isset($CourseIconData[0]) && !empty($CourseIconData) ? $CourseIconData[0]->id : '';

                                                $overallProgress = calculateOverallCourseProgress(
                                                    $MasterCourseData, 
                                                    $StudentId, 
                                                    $courseIconId, 
                                                    $watchContentIds
                                            ...
                                            <div class="mb-3 mt-3">
                                                <div class="progress" style="height: 6px">
                                                    <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated total_progress_display_value"
                                                        role="progressbar"
                                                        value="";
                                                        style="width:{{ isset($overallProgress['percentage'])  ?  $overallProgress['percentage'].'%' : ''}} "
                                                        aria-valuenow="10"
                                                        aria-valuemin="0"
                                                        aria-valuemax="100">
                                                    </div>
                                                </div>
                                                <small class="total_progress_display_complete"> {{ isset($overallProgress['percentage'])  ?  $overallProgress['percentage'] : '0'}} %
                                                    {{__('static.completed')}}</small>
                                            </div>
                                            </li>
                                        @php $i=1; $totalProgress = 0; 
                                        $totalSectionCount = 0;
                                        $specialAwardId = env('courseResearchID');
                                        $MasterCourseData = collect($MasterCourseData)
                                            ->sortBy(function($item) use ($specialAwardId) {
                                                return ($item['coursemodules'][0]['id'] == $specialAwardId) ? 1 : 0;
                                            })
                                            ->values();
                                        @endphp
                                        ...
                                        @php
                                        // $sectionData = getData('course_managment_master', ['section_id'], ['course_master_id' => $masterdata['coursemodules'][0]['id'], 'is_deleted' => 'No'],'','id','ASC');
                                        // if(!empty($sectionData) && isset($sectionData) && $sectionData->isNotEmpty()){
                                        //     if (isset($sectionData[0]->section_id)) {
                                        //         $VideoData = getData('course_modules_videos', ['bn_collection_id', 'bn_video_url_id', 'video_title', 'id', 'video_duration'], ['section_id' => $sectionData[0]->section_id, 'is_deleted' => 'No']);
                                        //         if ($VideoData->count() ==  0){
                                        //             $ContentData = getData('course_content_docs', [ 'file', 'docs_title', 'id', 'doc_file_name'], ['section_id' => $sectionData[0]->section_id, 'is_deleted' => 'No']);
                                        ...
                                        <ul class="list-group-item">
                                            ...
                                            @php
                                                $courseProgress = calculateCourseProgress(
                                                    $masterdata['course_manage'], 
                                                    $StudentId, 
                                                    $masterdata['course_id'],
                                                    $courseIconId,
                                                    $watchContentIds,
                                                    $masterdata['award_id']
                                                );  
                                            ...
                                            <div class="d-flex align-items-center justify-content-between w-100 mt-2">
                                                <div class="progress flex-grow-1 me-2" style="height: 6px;">
                                                    <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated"
                                                        role="progressbar"
                                                        style="width: {{ isset($courseProgress['percentage']) ? $courseProgress['percentage'].'%' : '0%' }};"
                                                        aria-valuenow="{{ isset($courseProgress['percentage']) ? $courseProgress['percentage'] : '0'}}" aria-valuemin="0" aria-valuemax="100">
                                                    </div>
                                                </div>
                                                <small class="text-muted fw-semibold" style="white-space: nowrap;">{{isset($courseProgress['percentage']) ? $courseProgress['percentage'] : '0'}}%</small>
                                            </div>
                                                ...
                                                <div class="py-3 nav" id="course-tabOne" role="tablist"
                                                    ...
                                                    <div class="accordion" id="sectionAccordion{{$i}}">
                                                        ...
                                                        <a class="mb-0 justify-content-between align-items-center text-inherit d-flex"
                                                            ...
                                                            href="#course{{$i}}course_section{{ $j }}" role="button"
                                                            ...
                                                            $sectionProgress = calculateSectionProgress(
                                                                $sections['sections'][0]['section_manage'], 
                                                                $StudentId, 
                                                                $masterdata['course_id'], 
                                                                $courseIconId, 
                                                                $watchContentIds,
                                                                $masterdata['award_id'],
                                                                $sections['sections'][0]['section_name']
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between w-100 mt-2">
                                                            <div class="progress flex-grow-1 me-2" style="height: 6px;">
                                                                <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated"
                                                                    role="progressbar"
                                                                    style="width: {{ isset($sectionProgress['percentage']) ? $sectionProgress['percentage'].'%' : '0%' }};"
                                                                    aria-valuenow="{{ isset($sectionProgress['percentage']) ? $sectionProgress['percentage'] : '0'}}" aria-valuemin="0" aria-valuemax="100">
                                                                </div>
                                                            </div>
                                                            <small class="text-muted fw-semibold" style="white-space: nowrap;">{{isset($sectionProgress['percentage']) ? $sectionProgress['percentage'] : '0'}}%</small>
                                                        </div>

                                                        <div class="collapse {{ $j === 0 ? 'show' : '' }}" id="course{{$i}}course_section{{ $j }}"
                                                                    ...
                                                                    @foreach ($sections['sections'][0]['section_manage'] as $key => $section_manage)
                                                                        @if (!empty($section_manage['content_type_id']) && $section_manage['content_type_id'] === 1)
                                                                            @foreach ($section_manage['course_video'] as $video)
                                                                                <a class="mb-0 d-flex justify-content-between align-items-center text-inherit
                                                                                studentAwardCourseTitle tab-link {{ $k === 0  ?  'active' : '' }}" 
                                                                                data-section-index="{{ $sectionIndex }}"
                                                                                id="course-project-tab" data-bs-toggle="pill"  href="#course-project"  onclick="videoDisplay('{{ $video['bn_video_url_id'] }}','{{$video['id']}}','{{base64_encode($masterdata['coursemodules'][0]['id'])}}')"  role="tab" aria-controls="course-project" aria-selected="false">
                                                                                    <div class="d-flex align-items-center">
                                                                                        <span class="icon-shape bg-green text-primary icon-sm rounded-circle me-2">
                                                                                            @php 
                                                                                            $CourseIdAward = $masterdata['award_id'];
                                                                                           
                                                                                            if(isset($CourseIconData[0]) && !empty($CourseIconData)){
                                                                                            $CourseProgressData = getData('video_progress',['full_check','watchedRanges','duration'],['video_id'=>$video['id'],'course_id' => $masterdata['award_id'],'user_id'=>$StudentId,'student_course_master_id'=>$CourseIconData[0]->id]);
                                                                                            }
                                                                                            @endphp
                                                                                            {{-- 
                                                                                            @if(!empty($CourseProgressData[0]))
                                                                                                ...
                                                                                                @endif
                                                                                            @else  --}}
                                                                                                <i id="play-pause-icon-{{ $video['id'] }}" class="checkifwatch-{{ $video['id'] }} bi bi-play-fill playIconStudentCoursePanel"></i>
                                                                                            {{-- @endif --}}
                                                                                        </span>
                                                                                        <div class="ms-1">
                                                                                ...
                                                                                <div class="text-muted mb-2" style="margin-left: 2.8rem;">
                                                                                    @php 
                                                                                    if(!empty($CourseProgressData[0]) && $CourseProgressData[0]->duration > 0 ){
                                                                                        $watchRanges = $CourseProgressData[0]->watchedRanges;
                                                                                        $duration = $CourseProgressData[0]->duration;
                                                                                        $fullCheck = $CourseProgressData[0]->full_check;
                                                                                        $result = calculateWatchProgress($watchRanges, $duration, $fullCheck);
                                                                                        $percentagVideo = $result['percentage'];
                                                                                        $videoWatch = $result['formatted'];
                                                                                    }else{
                                                                                        $percentagVideo = 0;
                                                                                        $videoWatch = formatTime(0).' / '.$video['video_duration'];
                                                                                    }
                                                                                    @endphp
                                                                                    <div class="mb-3 mt-3">
                                                                                        <!-- Top line: percentage (left) + time (right) -->
                                                                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                                                                            <small class="total_progress_display_complete" style="white-space: nowrap;">
                                                                                                {{ isset($percentagVideo) ? round($percentagVideo, 2) : '0' }}%
                                                                                            </small>
                                                                                            <small class="video_time_display" style="white-space: nowrap;">
                                                                                                {{ $videoWatch ?? '0:00 / 0:00' }}
                                                                                            </small>
                                                                                        </div>
                                                                                        ...
                                                                                        <div class="progress" style="height: 6px;">
                                                                                            <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated total_progress_display_value"
                                                                                                role="progressbar"
                                                                                                style="width: {{ isset($percentagVideo) ? $percentagVideo.'%' : '0%' }};"
                                                                                                aria-valuenow="{{ isset($percentagVideo) ? $percentagVideo : 0 }}"
                                                                                                aria-valuemin="0"
                                                                                                aria-valuemax="100">
                                                                                            </div>
                                                                                        </div>
                                                                                ...
                                                                                @php
                                                                                    $uniqueKey = 'video_' . $video['id']; // For video
                                                                                    if (!in_array($uniqueKey, $countedItems)) {
                                                                                        $countedItems[] = $uniqueKey;
                                                                                        $totalProgress++;
                                                                                    }
                                                                                @endphp
                                                                            @endforeach
                                                                        @elseif(!empty($section_manage['content_type_id']) && $section_manage['content_type_id'] === 2)
                                                                            @foreach ($section_manage['course_article'] as $docs)
                                                                                ...
                                                                                <a class=" mb-2 d-flex justify-content-between align-items-center text-inherit studentAwardCourseTitle tab-link " data-section-index="{{ $sectionIndex }}"
                                                                                    ...
                                                                                    <div class="d-flex align-items-center">
                                                                                        <span
                                                                                            ...
                                                                                            @php $percentageDocs = 0; @endphp
                                                                                            @if (!empty($CourseIconData[0]->watch_content_id)) 
                                                                                                @php $Watcharray = explode(",", $CourseIconData[0]->watch_content_id);
                                                                                ...
                                                                                <div class="text-muted mb-2" style="margin-left: 2.8rem;">
                                                                                    <div class="mb-3 mt-3">
                                                                                        ...
                                                                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                                                                            <small class="total_progress_display_complete" style="white-space: nowrap;">
                                                                                                {{ isset($percentageDocs) ? round($percentageDocs, 2) : '0' }}%
                                                                                            </small>
                                                                                        </div>
                                                                                        ...
                                                                                        <div class="progress" style="height: 6px;">
                                                                                            <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated total_progress_display_value"
                                                                                                role="progressbar"
                                                                                                style="width: {{ isset($percentageDocs) ? $percentageDocs.'%' : '0%' }};"
                                                                                                aria-valuenow="{{ isset($percentageDocs) ? $percentageDocs : 0 }}"
                                                                                                aria-valuemin="0"
                                                                                                aria-valuemax="100">
                                                                                            </div>
                                                                                        </div>
                                                                                        ...
                                                                                        <span
                                                                                            ...
                                                                                            @php  $Watcharray = explode(",", $CourseIconData[0]->watch_content_id); @endphp
                                                                                                @if (in_array('pd_'.$docs['id'], $Watcharray))
                                                                                                    ...
                                                                                                    class="bi bi-check2 nav-icon fs-6 color-blue playIconStudentCoursePanel"></i>
                                                                                                @else
                                                                                                    <i id="play-pause-icon-{{ $docs['id'] }}" data-filetype="pdf" class="checkifwatch-{{ $docs['id'] }} bi bi-file-earmark-pdf nav-icon fs-6 color-blue playIconStudentCoursePanel"></i>
                                                                                                @endif
                                                                                            @else
                                                                                                <i id="play-pause-icon-{{ $docs['id'] }}" data-filetype="pdf"
                                                                                ...
                                                                                <div class="text-muted mb-2" style="margin-left: 2.8rem;">
                                                                                    <div class="mb-3 mt-3">
                                                                                        ...
                                                                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                                                                            <small class="total_progress_display_complete" style="white-space: nowrap;">
                                                                                                {{ isset($percentageDoc) ? round($percentageDoc, 2) : '0' }}%
                                                                                            </small>
                                                                                        </div>
                                                                                        ...
                                                                                        <div class="progress" style="height: 6px;">
                                                                                            <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated total_progress_display_value"
                                                                                                role="progressbar"
                                                                                                style="width: {{ isset($percentageDoc) ? $percentageDoc.'%' : '0%' }};"
                                                                                                aria-valuenow="{{ isset($percentageDoc) ? $percentageDoc : 0 }}"
                                                                                                aria-valuemin="0"
                                                                                                aria-valuemax="100">
                                                                                            </div>
                                                                                        </div>
                                                                            ...
                                                                            @foreach ($section_manage['course_quiz'] as $quiz)
                                                                                ...
                                                                                <div class="text-muted mb-2" style="margin-left: 2.8rem;">
                                                                                    <div class="mb-3 mt-3">
                                                                                        ...
                                                                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                                                                            <small class="total_progress_display_complete" style="white-space: nowrap;">
                                                                                                {{ isset($percQuiz) ? round($percQuiz, 2) : '0' }}%
                                                                                            </small>
                                                                                        </div>
                                                                                        ...
                                                                                        <div class="progress" style="height: 6px;">
                                                                                            <div class="progress-bar bg-blue progress-bar-striped progress-bar-animated total_progress_display_value"
                                                                                                role="progressbar"
                                                                                                style="width: {{ isset($percQuiz) ? $percQuiz.'%' : '0%' }};"
                                                                                                aria-valuenow="{{ isset($percQuiz) ? $percQuiz : 0 }}"
                                                                                                aria-valuemin="0"
                                                                                                aria-valuemax="100">
                                                                                            </div>
                                                                                        </div>
                                                                                ...
                                                                                {{-- @php $totalProgress++; @endphp --}}
                                                                                @php
                                                                                    $uniqueKey = 'quiz_' . $section_manage['content_id'];
                                                                                    if (!in_array($uniqueKey, $countedItems)) {
                                                                                        $countedItems[] = $uniqueKey;
                                                                                        $totalProgress++;
                                                                                    }
                                        ...
                                        @php 
                                        $totalLock  = 0;
                                        if($CourseWatchDatas[0]->payment_installment_type == "InstallmentPayment" && $totalSectionCount > 0){
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="tab-content content" id="course-tabContent">
                        <div class="tab-pane fade {{ $VideoData && $VideoData->isNotEmpty() ? 'show active' : '' }}" id="course-project" role="tabpanel"
                      ...
                      @include('frontend/student/part-quiz-award-course-panel',['QuizCourse'=>$MasterCourseData])
                        ...
                        <!-- Resource 1 Tab pane -->
                        <div class="tab-pane fade  {{ $VideoData && $VideoData->isEmpty() ? 'show active' : ''}}" id="resource" role="tabpanel"
...
<script>
    ...
    const completedText = "{{ __('static.completed') }}";
...
<script>
    ...
    $(document).ready(function () {
        ...
        console.log("courseExpiryDate:", courseExpiryDate);
    ...
    document.addEventListener("DOMContentLoaded", function() {
        <?php if (!empty($VideoData) && isset($VideoData[0]->bn_video_url_id)) : ?>
        ...
        var lockVideos = isVideoLockedUser; 
        if(totalLock > 0){
        ...
        <?php else: ?>
        <?php if (!empty($ContentData) && isset($ContentData[0]->file)) : ?>
            var file = "<?php echo $ContentData[0]->file; ?>";
            var title = "<?php echo $ContentData[0]->docs_title; ?>";
            var pdid = "<?php echo $ContentData[0]->id; ?>";

            // PdfContentDisplay(defaultVideoId, defaultId);
            PdfContentDisplay(file, title,pdid);
        <?php endif; ?>
    ...
    function videoDisplay(videoId, vid) {
        ...
        const course_id = "<?php echo base64_encode($course_id); ?>";
        ...
        playerInstance.on('pause', () => {
            playerInstance.pause();
            console.log("Paused at watchTime:", watchTime);
        })
        ...
        function lasttimestore(course_id, vid, watchTime, hasFastForwarded, lastRecordedTime) {
            ...
            // console.log("lastrecord:",lastRecordedTime)
            // console.log("Watched Ranges:", watchedRanges);
            // console.log("Merged Ranges:", mergeRanges(watchedRanges));
            // console.log("Accurate Time:", calculateTotalWatchedTime(mergeRanges(watchedRanges)));
            // return;
                    ...
                    if(response.full_check == 'Yes'){
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                console.log('datatrue',response.data);
                                if(response.data == "FALSE"){
                                    ...
                                    console.log('datacount',progress_count_total);
                                    if(response.count == '0'){
                                    ...
                                    document.querySelector('.total_progress_display_complete').textContent =  total_progress_display.toFixed(0) + '% '+ completedText;
                    ...
                    success: function (response) {
                        ...
                        if(response.data != undefined){
                            response.data.forEach(item => {
                                ...
                                let icons = document.querySelectorAll(".checkifwatch-" + item.video_id);
    ...
    $(".quizActive").on("click", function (event) {
        ...
        if(course_id != undefined){
            ...
            const ementorBaseUrl = window.location.origin + "/ementor";
            var user_id = "<?php echo base64_encode($StudentId); ?>";
            $.ajax({
               url: ementorBaseUrl + "/quiz-views",
               type: "POST",
               data: { id: quiz_id,course_id: course_id,user_id:user_id},
               dataType: "json",
               headers: {
                   "X-CSRF-TOKEN": csrfToken,
               },
