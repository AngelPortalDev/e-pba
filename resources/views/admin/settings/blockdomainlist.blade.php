                    ...
                    <div class="mb-2 mb-lg-0">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="#">Domain BlockList</a></li>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="taskModalLabel">Add Domain to Block List</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form class="row domainBlockForm" novalidate>
                    <div class="mb-2 col-12">
                        <label for="domainInput" class="form-label">Domain <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="domainInput" name="domain" placeholder="e.g. example.com"
                               pattern="^([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$" required>
                        <div class="invalid-feedback" id="domain_error">Please enter a valid domain.</div>
                    </div>
                    <div class="col-12 d-flex justify-content-end pt-2">
                        <button type="button" class="btn btn-outline-secondary me-2" data-bs-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-primary adddomainblock">Add to Block List</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
   $.fn.dataTable.ext.errMode = 'none';
    ...
    function AllblockList(action){
        ...
        if ( $.fn.DataTable.isDataTable('.all_block_list') ) {
            $('.all_block_list').DataTable().destroy();
        }
        $.ajax({
            url: baseUrl + 'admin/get-domain/' + action,
            method: 'GET',
                ...
                $('.all_block_list').DataTable({
                    ...
                    columns: [
                        ...
                        "render": function(row) {
                                    ...
                                    var isBlocked = row.is_active == 1;
                                    var action = isBlocked ? 'unblock' : 'block';
                                    var label = isBlocked ? 'Unblock' : 'Block';
                                    return '<button onclick="toggleDomain(\'@' + domain + '\', \'' + action + '\')" class="btn btn-sm btn-' + (isBlocked ? 'danger' : 'success') + '">' + label + '</button>';
                                },
                            width: '20%'
                        }
                    ]
    ...
    function toggleDomain(domain, action) {
        const modalData = {
            title: (action === 'block') ? 'Block Domain' : 'Unblock Domain',
            message: `Are you sure you want to ${action} the domain ${domain} ?`,
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    const modalData = {
                        ...
                        icon: response.icon || successIconPath
                    };
                    showModal(modalData);
                    AllblockList('all'); // Refresh the list
                },
                error: function(xhr) {
